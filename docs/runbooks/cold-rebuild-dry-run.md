...
|-----|-------|--------|-----------|
| terraform.yml ephemeral keypair | [#216](https://github.com/noorinalabs/noorinalabs-deploy/issues/216) | [#217](https://github.com/noorinalabs/noorinalabs-deploy/pull/217) | static — `cold_rebuild_static_checks.py` asserts (a) terraform.yml contains no `ssh-keygen`, (b) both `modules/hetzner-vps/deploy.pub` and `modules/hetzner-vps/root.pub` exist and look like valid SSH key lines, (c) env-root `deploy_ssh_public_key_path` **and** `root_ssh_public_key_path` defaults each point at the canonical checked-in pubkey, never an operator-local `~/.ssh/...` path. Updated for the ADR 0006 per-env per-role split ([#164](https://github.com/noorinalabs/noorinalabs-deploy/issues/164)). |
