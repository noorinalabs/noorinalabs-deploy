...
- **Part-2 implementation issue:** [deploy#331](https://github.com/noorinalabs/noorinalabs-deploy/issues/331) — "Implement ADR 0004 — `terraform/backblaze-bootstrap/` root + annual-rotation calendar runbook + optional `rotate-b2-state-key.sh` automation." Lands the `terraform/backblaze-bootstrap/` module (Decision A2), the [`docs/runbooks/state-key-annual-rotation.md`](../runbooks/state-key-annual-rotation.md) runbook (Decision C), and downgrades [`docs/runbooks/state-bucket-lifecycle.md`](../runbooks/state-bucket-lifecycle.md) to spec-of-record + DR fallback. Calendar-reminder placement (Decision C) and the optional rotation-automation workflow (Decision C4) are deferred owner-decisions noted in the rotation runbook.
