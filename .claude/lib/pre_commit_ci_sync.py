...
Vendored from the parent (noorinalabs-main) canonical gate for the org rollout
(noorinalabs-deploy adoption, W14). The only delta vs the parent is the
`build`-kind classifier — narrowed so a real build-quality gate is still tracked
but runtime `docker build` / `docker buildx` image-moving is not misclassified
(see the `_KIND_PATTERNS` note below). This tightening is PROPOSED-CANONICAL:
it is being lifted upstream into the parent gate and re-vendored org-wide, since
the image-publishing repos (isnad-graph, ingest-platform) hit the same false
match. The gate is run UNSCOPED over all `.github/workflows/*.yml`.
...
# buildx`, so a repo that uses buildx at runtime (deploy, image-publishing CI
# in isnad-graph / ingest-platform) would otherwise see a permanent un-mirrorable
# "build" kind and the drift gate could never exit 0. If a repo ever adds a
# genuine docker-build-as-quality-gate, name that job `build-and-validate` /
# `build-and-test` (or add an explicit pattern) so it is mirror-tracked.
