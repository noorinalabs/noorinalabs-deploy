...
- [Prod hardening is unreachable in CI](feedback_prod_hardening_unreachable_in_ci.md) — backup unit's /tmp is READ-ONLY (ProtectSystem=strict, no PrivateTmp); CI/rehearsal/dev /tmp is writable → the whole test surface runs where the bug CANNOT occur. Why backups never once succeeded; why #613 shipped TWICE past green CI. Derive the sandbox FROM the unit file, never restate it (#626/#627). A check that cannot run must say so (rc=2 ≠ rc=1) and must NOT testify.
