...
- [stderr is COMMENTARY, not DATA](feedback_stderr_is_commentary_not_data.md) — `2>/dev/null`/`|| true` DISCARDS the diagnostic; `2>&1` into a variable that is then PARSED promotes it to DATA and **fires on the SUCCESS path** (tools write to stderr when nothing is wrong — rclone's config NOTICE became a "backup directory"). Capture with `2>"$err"`, read on failure. Fine when the var is only printed. deploy#584.
